...
const getIndexName = (arr: any[], ib: any) => {
  ...
  if (ib.k.includes('upsample_v6_2x'))
    return ib.n
...
<template>
  <div v-if="st.isLoadImg">
    <div v-if="chat.opt?.status == 'FAILURE'">
      <div>
        {{ $t('mjchat.failReason') }}<p>{{ chat.opt?.failReason }}</p>
      </div>
    </div>
    <template v-else-if="chat.opt?.progress">
      ...
      <NImage
        v-if="chat.opt.imageUrl" :src="st.uri_base64 ? st.uri_base64 : mjImgUrl(chat.opt.imageUrl)"
      ...
      <div v-if="chat.opt?.status == 'SUCCESS'" class=" space-y-2">
        <template v-if="chat.opt?.buttons">
          <div v-for="(bts, ii) in bt" :key="ii" class=" flex justify-start items-center flex-wrap ">
            <template v-for="ib in bts">
              <div v-if="getIndex(chat.opt?.buttons, ib) > -1">
                <button
                  class="mj-btn"
                  :data-style="chat.opt?.buttons[getIndex(chat.opt?.buttons, ib)].style" size="small"
                  @click="subV2(ib)"
                >
        ...
        <template v-else>
          ...
          <div class="flex space-x-2">
            ...
            <NButton
              v-if="chat.opt?.action === 'IMAGINE'" type="warning" size="small"
              @click="sub('REROLL', 1)"
            >
              {{ $t('mjchat.reroll') }}
            </NButton>
    ...
    <NModal
      v-model:show="st.isShow" preset="card" :title="$t('mjchat.redrawEditing')" style="max-width: 800px;"
      @close="st.isShow = false"
    >
      <aiCanvas v-if="st.isShow" :chat="chat" :base64="st.uri_base64" @success="maskOk" />
    </NModal>
    <NModal
      v-model:show="st.isCustom" preset="card" :title="$t('mj.customTitle')" style="max-width: 600px;"
