...
const fsRef = ref()
const st = ref<{ fileBase64: string[], fileName: string[], isLoad: number, isShow: boolean, showMic: boolean, micStart: boolean }>({
    fileBase64: [], fileName: [], isLoad: 0
    ...
    <div class="  myinputs" @drop="drop" @paste="paste" v-else>
            ...
            <!-- <div class="absolute bottom-0 right-0 z-1">
                <NPopover trigger="hover">
                    <template #trigger>
                        <NTag type="info" round size="small" style="cursor: pointer; " :bordered="false">
                            <div class="opacity-60 flex">
                                <SvgIcon icon="material-symbols:token-outline" /> {{ $t('mj.remain') }}{{ myToken.remain
                                }}/{{ myToken.modelTokens }}
                            </div>
                        </NTag>
                    </template>
                    <div class="w-[300px]">
                        ...
                        <p class=" text-right">
                            <NButton @click="st.isShow = true" type="info" size="small">{{ $t('setting.setting') }}
                            </NButton>
                        </p>
                    </div>
        ...
        <NAutoComplete v-model:value="mvalue" :options="searchOptions" :render-label="renderOption">
            <template #default="{ handleInput, handleBlur, handleFocus }">
                <NInput size="large" style="border-radius: 24px;" ref="inputRef" v-model:value="mvalue" type="textarea"
                    ...
                    <template #prefix>
                        <div class="flex flex-row items-center">
                            <n-tooltip trigger="hover">
                                <template #trigger>
                                    <div class="flex flex-row items-center">
                                        <SvgIcon  v-if="st.isLoad == 1" icon="line-md:uploading-loop" class="cursor-pointer text-xl"
                                            >
                                        </SvgIcon>
                                        <SvgIcon v-else  icon="ri:attachment-line" class="cursor-pointer text-xl" @click="fsRef.click()">
                ...
                <!-- 语音 -->
                    ...
                    <template #suffix>
                        ...
                        <div class="flex py-1.5">
                            <NButton color="#000" circle type="default"
                                :disabled="disabled || homeStore.myData.isLoader" @click="handleSubmit">
                                <template #icon>
                                    <span class="dark:text-black">
                                        <SvgIcon icon="ri:stop-circle-line" v-if="homeStore.myData.isLoader" />
                                        <SvgIcon icon="ri:arrow-up-line" v-else />
                                    </span>
                                </template>
                            </NButton>
                        </div>
